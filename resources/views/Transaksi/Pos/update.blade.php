		...
		<thead> -->
			...
			<!-- form start -->
              <form method="Post" action="SalesUpdate">
                ...
                <!-- <div class="card-body"> -->
                	@csrf <input class="form-control" type="hidden" name="notaid" value="{{ $sales->Nota_ID }}">
               <table width="35%">
                  ...
                  <div class="form-group">
                    ...
                    <td><select name="customerid" class="form-control mb-1">
                      @foreach($customer as $C)
                       @if($C->Customer_ID == $sales->Customer_ID) 
                      <option value=" {{ $C->Customer_ID}}" selected>
				 ...
				 <div class="form-group">
                    ...
                    <input class="form-control" type="date" name="notadate" value="{{ $sales->Nota_Date }}">
                  </div> 
                  </td>                 	
              </div>
              </tr>
		...
		<tbody>
			 @foreach($sales_detail as $salesdetail)
                      <tr id="col{{ $loop->iteration }}">
                       ...
                       <td> <input type="hidden" name="id[{{ $salesdetail->Product_ID }}]" value="{{ $salesdetail->Product_ID }}">{{ $salesdetail->Product_Name }}</td>
		<td><input type="number" name="qty[{{ $salesdetail->Product_ID }}]" value="{{ $salesdetail->Quantity }}" oninput="recount({{ $salesdetail->Product_ID }})" id="qty{{ $salesdetail->Product_ID }}"></td>
		<td><input class="discount" type="number" name="discount[{{ $salesdetail->Product_ID }}]" value="{{ $salesdetail->Discount }}" oninput="recount({{ $salesdetail->Product_ID }})" id="discount{{ $salesdetail->Product_ID }}" style="-webkit-appearance: input;"></td>
		<td><input type="hidden" id="harga{{ $salesdetail->Product_ID }}" name="harga[{{ $salesdetail->Product_ID }}]" value="{{ $salesdetail->Selling_Price }}">{{ $salesdetail->Selling_Price }}</td>
		<td><input type="hidden" class="subtotal" name="subtotal[{{ $salesdetail->Product_ID }}]" value="{{ $salesdetail->Total_Price }}" id="subtotal{{ $salesdetail->Product_ID }}"><span id="subtotalval{{ $salesdetail->Product_ID }}">{{ $salesdetail->Total_Price }}</span></td>
		<td><button class="btn btn-danger" onclick="hapusEl('col{{ $loop->iteration }}')">Delete</button></td>
...
<script>
	...
	function pilihBarang(id){
		...
		cell3.innerHTML = '<input class="discount" type="number" name="discount['+barang[index]["Product_ID"]+']" value="1" oninput="recount('+barang[index]["Product_ID"]+')" id="discount'+barang[index]["Product_ID"]+'" style="-webkit-appearance: input;">';	
		...
		cell6.innerHTML = '<button onclick="hapusEl(\''+index+'\')">Del</button>';
